...
const PointsLayer = ({ selectedDatasource, limit, order }) => {
  ...
  useEffect(() => {
    ...
    let markers = data.map((d) => ({ ...d, color: coloring(d) })).map((d) => buildMarker(d))
...
const Map = ({ selectedDatasource, limit, order }) => {
  return (
    <MapContainer center={centerOfUkraine} zoom={6} worldCopyJump scrollWheelZoom className={css.map}>
      <TileLayer
