use crate::config::ImportFormat;
use crate::db::DbPool;
use geojson::{FeatureCollection, GeoJson};
use serde_json::Value;
use std::convert::TryFrom;

pub fn import(from: String, format: ImportFormat, db: DbPool) -> anyhow::Result<()> {
    match format {
        ImportFormat::GeoJSON => import_geo_json(from, db),
    }
}

fn import_geo_json(from: String, db: DbPool) -> anyhow::Result<()> {
    let conn = db.get()?;
